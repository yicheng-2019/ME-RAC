...
class MultiPath3dConv_Encoder_RAC(nn.Module):
    """ME-RAC: Multipath 3D-Conv Encoder for Repetitive Action Counting.

    This model replaces the standard single-path 3D convolution in TransRAC
    with a multipath architecture using parallel convolution kernels of
    different sizes (1x1x1, 3x3x3, 5x5x5) to capture richer temporal-spatial
    ...
    def __init__(self, config, checkpoint=None, num_frames=64, scales=None, OPEN=False):
...
if __name__ == '__main__':
    ...
    model = MultiPath3dConv_Encoder_RAC(config=config)
    model = model.to(device)

    input_tensor = torch.randn(2, 3, 64, 224, 224).to(device)  # [N, C, T, H, W]

    with torch.no_grad():
        model.eval()
        density_map, sim_matrix = model(input_tensor)

    print(f"Density map shape: {density_map.shape}")       # [2, 64]
    print(f"Similarity matrix shape: {sim_matrix.shape}")   # [2, 12, 64, 64]
